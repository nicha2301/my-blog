...
import { getAllPosts, getAllCategories } from "@/app/lib/sanity/api";
...
function HomeContent() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Lấy 2 bài viết đầu tiên làm featured
        setFeaturedArticles(posts.slice(0, 2).map(post => ({
          title: post.title,
        ...
        // Chuyển đổi dữ liệu danh mục để phù hợp với giao diện
        setCategories(fetchedCategories.slice(0, 3).map(category => ({
          id: category.slug,
          name: category.title,
          description: category.description || "Explore articles in this category",
          icon: getCategoryIcon(category.title) // Hàm helper để lấy icon theo tên danh mục
        })));
        ...
        setFeaturedArticles(sampleFeaturedArticles);
  ...
  }, [isLoading]); // Thêm isLoading vào dependencies để đảm bảo animation chạy sau khi dữ liệu đã được tải

  // Helper function to get icon based on category name
  function getCategoryIcon(categoryName) {
    const name = categoryName.toLowerCase();
    if (name.includes("design") || name.includes("principle")) {
      return (
    ...
    } else if (name.includes("interact") || name.includes("ux")) {
      return (
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 mx-auto">
...
// Sample data to fallback in case of error
