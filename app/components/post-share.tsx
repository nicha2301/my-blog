...
export function PostShare({ title, url, position = "vertical" }: PostShareProps) {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Nếu không có URL được truyền vào, sử dụng URL hiện tại
  ...
  // Nếu không ở client-side, không render gì cả
  ...
  const shareLinks = [
    {
      ...
      href: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(pageUrl)}&quote=${encodeURIComponent(title)}`,
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      ...
      <div className={position === "horizontal" ? "flex space-x-2" : "flex flex-col space-y-2"}>
        {shareLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
