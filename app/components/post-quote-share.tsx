...
interface PostQuoteShareProps {
  selector: string; // CSS Selector cho phần nội dung
}

export function PostQuoteShare({ selector }: PostQuoteShareProps) {
  ...
  const shareToTwitter = () => {
    ...
    const maxLength = 280 - currentUrl.length - 20; // Trừ độ dài URL và các ký tự thêm vào
    const quote = selectedText.length > maxLength 
      ? selectedText.substring(0, maxLength - 3) + "..." 
      : selectedText;
      
    const shareUrl = `https://twitter.com/intent/tweet?text="${quote}" ${currentUrl}`;
